...
本節ではスコープと代入を導入します。少々長い節になりますが、重要かつ最初はちょっとわかりづらいところを丁寧に説明するためですのであせらずゆっくり読んでください。


代入は、すでに存在する変数の値を変更することを言います。スコープとは変数をその外から見えないように閉じ込めた、ある範囲のことを言います。スコープも変数定義と同様 Python ではそれほど意識されていないかもしれません。というのは、Python には明示的にスコープを作る構文がないからです。といっても Python にスコープが存在しないわけではなく、関数やメソッドを書くとスコープが作られます。
...
[^more-parts]: 1 章の範囲では。3 章で作る中間コード式インタプリタではもうひとつ、呼び出した先の関数から戻ってくるためのしくみが必要になります。

一方、Java や JavaScript などの言語では `{}` で囲むとスコープができます [^javascript-scope]。この構文を使ってもうすこしスコープと代入の持つ性質を説明します。

[^javascript-scope]: JavaScript ではややこしいところもあるんですがとにかくできることはできます。

スコープの中にいるときは、外の変数が見えます。変数に値を代入すると、外の変数の値が変更されます。Python だと例示しづらいので JavaScript で。
