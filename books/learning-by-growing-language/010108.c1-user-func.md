...
無名関数について、前節の説明をさらっと復習します。Python の書き方で説明していますが、Toil の `<式>` も考え方は同じで書き方が違うだけです。
...
無名関数の式は `("func", [<仮引数の配列>, <本体>])` という形で書くことにします。`<本体>` は式ですが `<仮引数の配列>` はただ仮引数の名前が並んだ配列です。注意してください。`a` と `b` ふたつの引数を取ってその和を返す関数、Python で言えば `lambda a, b: a + b` となる関数は `("func", [["a", "b"], ("add", ["a", "b"])])` です。
...
ユーザ定義関数の呼び出し[^function-application]は、組み込み関数の呼び出しと同じ形（`(<関数>, [<式>, <式>, <式>, ...])`）です。`(lambda a, b: a + b) (2, 3)` に相当するものは `(("func", [["a", "b"], ("add", ["a", "b"])]), [2, 3])` となります。だいぶ見づらくなってきましたががんばってください。`("func", [["a", "b"], ("add", ["a", "b"])])` に `myadd` という名前がついていれば `("myadd", [2, 3])` となります。落ち着いて見比べれば大丈夫です。
...
これはある意味悪名高いグローバル変数が与える悪影響に近いものです。これは、動的スコープと呼ばれるしくみのせいなのです[^no-outer-scope]。

[^no-outer-scope]: そんなによくないものなら最初から外側のスコープへのアクセスなんか禁止しちゃえばいいのに、と思うかもしれません。しかしそうするとあらゆる入力を引数で与えなければならず、面倒になる場合が生じます。さらに、Toil では組み込み関数も（もっとも外側のスコープで定義された）単なる変数であることを思い出してください。外側のスコープへのアクセスを禁止したとたん、`add` も `equal` も、すべての組み込み関数が（引数として渡してやらない限り）使えなくなってしまいます。
