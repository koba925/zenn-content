...
そういえば、`def` に関しては何もしていないのに使えてますね。これは `def` が構文糖衣で、構文解析の時点で `define` と `func` に変換されるためです。
...
処理系のプログラムはツリーウォーク式も中間コード式も両方入ってまだ 500 行程度で、まだまだ上から下まで全部読むというのが無理なくできる大きさです。細かいステップに分けて進めてきましたので、一度全体を読み直してみると全体像の理解がよりいっそう深まると思います。
