...
次は if 式を導入します。文法は `if <Cond式> then <Then式> else <Else式> end` です。AST では `("if", [<Cond式>, <Then式>, <Else式>])` となります。`if <Cond式> then <Then式> end` のように `else` を省略したり、`elif` で続けて書いたりできるようにはしません（第 4 章でやります）。

これも `scope ... end` 同様、ちょっと変わったカッコと見て、`_primary()` に作ります。中身が三つあるカッコですね。
