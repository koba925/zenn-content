...
そこで、識別子や記号などを表現するための専用の型として `Ident` クラスを作ります。これらをすべて `Ident` 型にしておくことで、文字列と明確に区別できるようになります。識別子をそのままにしておいて文字列をたとえば `ToilStr` といったクラスで表すことによって区別することもできますが、数や `bool` のように、Python の値をそのまま Toil の値として使うことでシンプルな実装が可能になっていることもあり、文字列を `str` で表す方を選びました。
...
元が文字列なので `Ident` を `str` のサブクラスにするというのも素直なやり方なのですが、サブクラスにしてしまうと `case str()` に `Ident` オブジェクトもマッチしてしまい、ちょっと書きづらくなってしまうのでサブクラスにはせず、独立したクラスとしました。
...
ほとんどの箇所は `"..."` を `Ident("...")` にするだけですので、VS Code を使っているなら、正規表現を有効にして `"[^ ]+"` を `Ident($0)` を置換するようにすれば多少手間が減らせると思います。残念ながら識別子以外の文字列にもマッチしますのでひとつずつ置換したりスキップしたりしていく必要があります。他のエディタでもたいていは同じことができると思いますが、正規表現（特に `$0` の書き方）が違っているかもしれません。
...
実行例です。テストも兼ねて 3.12 実行例をそのまま残しておいてもいいでしょう。なんなら 2.18 のものを追加しても[^little-changes-in-tests]。
...
`Ident` クラスで定義した `__hash__` や `__repr__` のような、名前がふたつのアンダースコアで囲まれたメソッドを特殊メソッドと呼びます。これらを自作クラスに定義することで、Python の組み込み関数や `==` などの演算子の挙動を相当程度カスタマイズできます。
