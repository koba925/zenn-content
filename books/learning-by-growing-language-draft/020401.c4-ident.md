...
実行例はテストもかねて 3.12 のものをそのまま残しておくといいでしょう。なんなら 2.18 のものを追加しても。テストコード（test_twi.py や test_ici.py）を使っている人は既存のテストコードがそのまま動作することを確認してください。

動きましたか？ソースコードのあちこちを `Ident` でラップするだけの単調な作業でしたが、これで文字列など新しいデータ型を迎え入れる準備が整いました・・・が、全部正しく書き換えられたかちょっと不安なのではないでしょうか？次節では型ヒントを導入してチェックを手伝ってもらいます。
