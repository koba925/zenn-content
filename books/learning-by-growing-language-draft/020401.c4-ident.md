...
そこで、識別子や記号などを表現するための専用の型として `Ident` クラスを作ります。これらをすべて `Ident` 型にしておくことで、文字列と明確に区別できるようになります。識別子をそのままにしておいて文字列をたとえば `ToilStr` といったクラスで表すことによって区別することもできますが、数や `bool` のように、Python の値をそのまま Toil の値として使うことでシンプルな実装が可能になっていることもあり、文字列を `str` で表し、識別子のために別のクラスを定義する方を選びました。
...
識別子が `Ident` 型になったので、トークンとの比較や `match` を `Ident("...")` に対して行うように修正しました。式の種別を示す識別子も `(Ident("seq"), exprs)` のように `Ident` 型にします。
...
`bind()` では `params` に `Ident` 型で仮引数が入っていることを期待して、そのまま `define()` に渡しているので修正は不要です。
...
`a := 2` の AST の `print` 結果を見てください。`define` や `a` がクォーテーションで囲まれていません。今後、Toil で文字列を扱うようになったときはクォーテーションで囲まれていれば文字列、囲まれていなければ識別子（`Ident`）であると見分けられます。これが `Ident` の `__repr__()` や `__str__()` をカスタマイズした目的です。ただしひとつ注意点があります。`print("aaa")` のように裸の文字列を `print` したときだけは文字列にクォーテーションがつきません（配列やタプルの中の文字列はクォーテーションで囲まれます）。たいていの場合は問題ないのですが、どうしても裸の文字列と識別子を表示で区別したいときは `repr()` で囲むなどして `repr` 形式で表示してください。

いつも前回の実行例を消している方も、今回は処理系自体がちゃんと動くか確かめるために 3.12 節の実行例をそのまま残しておきましょう。`run()` の実行例しかありませんので、 2.18 の `walk()` の実行例も追加して確かめておくとより安心です。test_twi.py や test_ici.py も `Ident` を使うように修正していますので心配な方はこちらでテストしてみてください[^little-changes-in-tests]。

[^little-changes-in-tests]: test_twi.py や test_ici.py のテストコードには処理系本体と違ってテストにはほとんど修正点がありません。これは test_twi.py や test_ici.py のテストが `walk()` や `run()` を中心としたテストになっていて、AST のような処理系の内部に関するテストを行っていないからです。ユニットテストとしてはテストが足りていませんが、内部表現に依存しないことにより変更に強いテストになっているとも言えます。
