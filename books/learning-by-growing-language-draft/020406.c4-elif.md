...
ついでに、Else 節を省略できるようにもします。Toil では `if` も式なので、Else 節がなかったからと言って何も返さないわけにはいきません。Else 節がなくて条件が真でない場合は `None` を返すことにします。
...
`elif` だったときがミソなんですが、`else_expr = self._if()` で済んでしまいます。

`if True then 2 elif True then 3 end` を例にとり、`elif` と Else 節省略をまとめて見ていきましょう。
...
```py
    ...
    print("Else and omitting else clauses:")
