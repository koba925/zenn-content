...
`tuple()` でタプルを生成します。`args` には配列が渡されるのでタプルに変換しているだけです。さっそく実行例を見てみましょう。
...
`len()` と `slice()` は基本的な組み込み関数として用意したものです。`slice()` は Python の `[start:stop]` 表記に対応したもので、`start` や `stop` に `None` を指定すると先頭・末尾を指定したことになります。
...
これで使えることは使えるんですが、長いので一般的な言語と同様 `[]` で要素にアクセスできるようにします。`tpl[2]` としたらタプルの 3 つめの要素を参照します。普通です。ただし、スライス表記（`tpl[2:5:2]` のような表記）はサポートしません。`slice()` を使ってください。Python と同じルールで書けるようにしようとすると省略の対応とかけっこう大変なので。読者の課題とします（丸投げ
...
「2.18 いろいろ書いてみる（再）」の二分探索木の例ではノードを表現するためにクロージャを使っていましたが、タプルで書けるようになりました。例によって二分探索木については説明しません。
...
組み込み関数の追加と、字句解析・構文解析の修正でタプルを追加できました。
